...
MathJax.Hub.Config({
  tex2jax: {inlineMath: [['$','$'], ['\\\\(','\\\\)']]}
       ...
       (<body>
        ...
        (<p> "NumPy uses powerful index notation for specifying subarrays, notation along the lines of "(<code>"start:end:skip")" to specify the beginning index along an axis of a subarray, the end index, and the number of elements to skip between successive indices.  In SRFI 231  "(<code>'array-extract)" specifies subarrays, and "(<code>'array-sample)" samples elements that are uniformly spaced in an array; the latter routine requires that an array's lower bounds all be zero.")
